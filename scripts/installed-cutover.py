...
def operational_receipt(*, inventory: Mapping[str, Any], selector: Mapping[str, Any], canary: Mapping[str, Any], backup_ref: str, rollback_results: list[Mapping[str, Any]]) -> dict[str, Any]:
    result = {"schema_version": 1, "kind": "ask-park-installed-cutover", "repository_identity": "zinan92/wechat-miniprogram-shipping", "inventory_digest": MIGRATION_MODULE._hash_json(inventory), "selector": {"ask_park_enabled_count": selector["ask_park_enabled_count"], "legacy_enabled_count": selector["legacy_enabled_count"], "one_canonical": selector["one_canonical"]}, "canary": copy.deepcopy(dict(canary)), "backup_ref": backup_ref, "rollback_results": copy.deepcopy(rollback_results), "evidence_mode": "sanitized-persisted", "limitations": ["Installed selector state is local to the scanned roots read during this operation."]}
